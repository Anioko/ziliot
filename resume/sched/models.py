...
ROLE_USER = 1
ROLE_COMPANY_FREE = 2
ROLE_COMPANY_BASIC = 3
ROLE_COMPANY_SILVER = 4
ROLE_COMPANY_GOLD = 5
ROLE_COMPANY_PLATINUM = 6
ROLE_SCHOOL = 7
ROLE_CANDIDATE = 8
...
positions_resumes = db.Table('positions_resumes',
        db.Column('resume_id', db.Integer(), db.ForeignKey('resumes.id')),
        db.Column('position_id', db.Integer(), db.ForeignKey('Positions.id')))
...
class Resume(db.Model):
    ...
    positions = relationship('Position', secondary=positions_resumes, 
                            backref=backref('Position', lazy= 'dynamic'))
...
class Position(db.Model):
    ...
    users = relationship('User', secondary=postions_users,
                            backref=backref('User', lazy='dynamic'))
    resumes = relationship('Resume', secondary=positions_resumes, 
                            backref=backref('Resume', lazy= 'dynamic'))
...
class Subscription(db.Model):
    ...
    subs_type = Column(String(255))
    amount = Column(Integer)

    user_id = db.Column(db.ForeignKey('user.id'))
    user = db.relationship('User')

    stripe_subs_id = Column(String(255))
    stripe_cust_id = Column(String (255))
...
class CompanyUserData(db.Model):
    ...
    subs_id = db.Column(db.Integer, db.ForeignKey('subscription.id'))
